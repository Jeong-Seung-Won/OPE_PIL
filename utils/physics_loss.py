...
class SatellitePhysicsLoss(nn.Module):
    ...
    def __init__(self, 
        ...
        if verbose:
            print(f" SatellitePhysicsLoss initialized:")
    ...
    def _second_derivative(self, x):
        """Second-order difference: x[t+1] - 2*x[t] + x[t-1]."""
        if x.size(1) < 3:
    ...
    def _gradient_magnitude(self, x):
        """Absolute first-order difference (right-padded)."""
        if x.size(1) < 2:
...
class PhysicsAwareCriterion:
    ...
    def get_detailed_loss(self, x_pred, x_true):
        """For inspecting individual loss components."""
